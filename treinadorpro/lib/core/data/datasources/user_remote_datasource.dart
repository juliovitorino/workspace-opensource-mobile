...
class UserRemoteDatasource implements IUserRemoteDataSource {
  ...
  Future<UserModel> fetchByUUID(String uuid) async {
    final String url =
        '${config.apiBackendUrl}/v1/api/business/user/trainer/$uuid';
  ...
  Future<RegisterResponse> register(RegisterRequest request) async {
    final String url =
        "${config.apiBackendUrl}/v1/api/business/public/user/register";
  ...
  Future<bool> validateCode(String trainerExternalId, String code) async {
    final String url =
        "${config.apiBackendUrl}/v1/api/business/public/user/trainer/validate/$trainerExternalId/$code";
  ...
  Future<String> login(LoginRequest loginRequest) async {
    final String url =
        "${config.apiBackendUrl}/v1/api/business/public/user/login";
  ...
  @override
  Future<ApiGenericResponse<TrainerAvailableTimeResponseModel>>
  findTrainerAvailableTime() async {
    final String url =
        '${config.apiBackendUrl}/v1/api/business/user/trainer/availabletimes';
    ...
    final responseModel = response['response'];
    final TrainerAvailableTimeResponseModel trainerAvailableTimes = TrainerAvailableTimeResponseModel.fromJson(
      response['objectResponse'],
    );

    return ApiGenericResponse(
      Response(responseModel['msgcode'], responseModel['mensagem']),
      trainerAvailableTimes,
    );
  }
}
