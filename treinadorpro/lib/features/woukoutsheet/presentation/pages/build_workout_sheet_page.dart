...
import 'package:treinadorpro/features/woukoutsheet/presentation/blocs/build_workout_sheet_cubit.dart';
...
class _BuildWorkoutSheetPageState extends ConsumerState<BuildWorkoutSheetPage> {
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Grupo Muscular'),
            workgroupState.when(
              data: (workgroupList) => _buildWorkgroupSearchable(workgroupList),
              error: (e, _) => Center(child: Text('Error: $e')),
              loading: () => Center(child: CircularProgressIndicator()),
            ),
                  ...
                  setState(() {
                    ...
                    userWorkoutPlanData[_workGroup.namePt]!.add(
                      UserWorkoutPlanModel(
                        ...
                        goal: _goal,
                        program: _program,
                        customProgram: _customProgram != null
                            ? _customProgram
                            : null,
                        //colocar
            ...
            ElevatedButton.icon(
              onPressed: () => _showAlertDialogSaveUserDataSheet(context),
              icon: Icon(Icons.check),
              label: Text('Gravar Ficha de Treino'),
  ...
  void _showAlertDialogClose(BuildContext context, String title) {
    showDialog(
      context: context,
  ...
  void _showAlertDialogSaveUserDataSheet(BuildContext context) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => ProWidgetAlertDialog(
        title: 'Salvar a ficha de treino definitiva?',
        proceedButton: 'Salvar',
        onProceed: () async {
          final UserDataSheetPlanModel? request = await _userPlanDraft.get(_contract.externalId);
          if(request != null) {
            context.read<BuildWorkoutSheetCubit>().saveUserDataSheetPlan(request);
            _userPlanDraft.clear(_contract.externalId);
            _contractTokenStorage.save(_contract.externalId);
            Navigator.of(context).pop(); // close dialog
          } else {
            Navigator.of(context).pop(); // close dialog
            _showAlertDialogClose(context, 'Não existe rascunho');
          }
        },
        onCancel: () {
          Navigator.of(context).pop(); // close dialog
